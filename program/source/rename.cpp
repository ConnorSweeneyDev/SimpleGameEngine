...
namespace sdl
{
    ...
    int get_desktop_display_mode(int display_index, Display_mode* mode) { return SDL_GetDesktopDisplayMode(display_index, mode); }
    Window* create_window(const char* title, int x, int y, int w, int h, Uint32 flags) { return SDL_CreateWindow(title, x, y, w, h, flags); }
...
namespace gl
{
    ...
    void get_shader_info_log(Uint shader, int max_length, int* length, char* info_log) { glGetShaderInfoLog(shader, max_length, length, info_log); }
    ...
    void buffer_data(Enum target, Size_i size, const void* data, Enum usage) { glBufferData(target, size, data, usage); }
    void vertex_attrib_pointer(Uint index, int size, Enum type, bool normalized, Size_i stride, const void* pointer) { glVertexAttribPointer(index, size, type, normalized, stride, pointer); }
    ...
    void tex_image_2d(Enum target, Int level, Int internal_format, Size_i width, Size_i height, Int border, Enum format, Enum type, const Void* pixels) { glTexImage2D(target, level, internal_format, width, height, border, format, type, pixels); }
...
namespace stbi
{
    ...
    unsigned char* load(const char* filename, int* width, int* height, int* channels, int desired_channels) { return stbi_load(filename, width, height, channels, desired_channels); }
