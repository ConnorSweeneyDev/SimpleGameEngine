...
HPP_INCLUDES := $(wildcard $(PROGRAM_INCLUDE_DIRECTORY)/*.hpp)
TPL_INCLUDES := $(wildcard $(PROGRAM_INCLUDE_DIRECTORY)/*.tpl.hpp)
INL_INCLUDES := $(wildcard $(PROGRAM_INCLUDE_DIRECTORY)/*.inl.hpp)
...
$(OBJECT_DIRECTORY)/%.o: $(PROGRAM_SOURCE_DIRECTORY)/%.cpp $(PROGRAM_INCLUDE_DIRECTORY)/%.hpp $(PROGRAM_INCLUDE_DIRECTORY)/%.inl.hpp | directories
	@$(CXX) $(CXXFLAGS) $(WARNINGS) $(INCLUDES) $(SYSTEM_INCLUDES) -c $< -o $@
	@$(ECHO) "CXX   | $@"
$(OBJECT_DIRECTORY)/%.o: $(PROGRAM_SOURCE_DIRECTORY)/%.cpp $(PROGRAM_INCLUDE_DIRECTORY)/%.tpl $(PROGRAM_INCLUDE_DIRECTORY)/%.inl.hpp | directories
	@$(CXX) $(CXXFLAGS) $(WARNINGS) $(INCLUDES) $(SYSTEM_INCLUDES) -c $< -o $@
	@$(ECHO) "CXX   | $@"
